...
int             pl_rc_file_access(const char *path, int amode);
...
void *          pl_rc_file_mmap(void * addr, int len, int prot, int flags, ts_pl_rc_file * fd, int offset);
...
int             pl_rc_file_stat_path(const char * path, ts_pl_rc_stat * buf);
